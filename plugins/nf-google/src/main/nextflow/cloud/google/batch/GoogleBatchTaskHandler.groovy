...
class GoogleBatchTaskHandler extends TaskHandler implements FusionAwareTask {
    ...
    boolean checkIfCompleted() {
        ...
        if( state in COMPLETED ) {
            ...
            // finalize the task
            // the `.exitcode` file may not be visible yet (e.g. GCS-backed work directory
            // propagation delay) even though the job has terminated -- wait for it instead
            // of failing the task
            final exitCode = getExitCode()
            if( exitCode == null )
                return false
            ...
            if( state == 'FAILED' ) {
                ...
                if( task.exitStatus >= 50000 ) {
                    final refreshed = readExitFile()
                    if( refreshed == null )
                        return false
                    task.exitStatus = refreshed
                }
     ...
     *
     * @return
     *      the exit code if found, {@code null} if the `.exitcode` file fallback is
     *      still within its retry window (caller should treat the task as
     *      not-yet-complete), otherwise {@link Integer#MAX_VALUE}
     */
